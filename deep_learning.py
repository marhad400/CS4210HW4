...
# SPECIFICATION: Iterating through different NN architectures and evaluating the performance of each, finding the highest accuracy.
...
def build_model(n_hidden, n_neurons_hidden, n_neurons_output, learning_rate):
    ...
    opt = keras.optimizers.legacy.SGD(learning_rate)
...
output_neurons = len(class_names)
...
for num_layers in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for neurons in n_neurons:                      #looking or the best parameters w.r.t the number of neurons
        for rate in l_rate:                  #looking or the best parameters w.r.t the learning rate
            ...
            #model = build_model()
            #-->add your Pyhton code here
            model = build_model(num_layers, neurons, output_neurons, rate)
            ...
            correct = 0
            total = 0
            for (y_testSample, prediction) in zip(y_test, class_predicted):
            ...
            print("Parameters: " + "Number of Hidden Layers: " + str(num_layers) + ",number of neurons: " + str(neurons) + ",learning rate: " + str(rate))
            print()
